...
- ~~**`ACCESS_FINE_LOCATION` is over-requested**~~ — **RESOLVED 2026-08-09.** The app never requested FINE; the MapLibre SDK's own manifest merged it (and `ACCESS_WIFI_STATE`) into the shipped bundle. Both are now stripped with `tools:node="remove"`, and `scripts/check-merged-manifest.sh` fails the build if either reappears. See `docs/PLAY-STANDARDS-AUDIT.md` H1.
- **RBN uses cleartext telnet** (`RbnSource.kt:25`) — **RESOLVED 2026-08-09.** `loginCall` defaults to `N0CALL` and `RbnPrivacyTest` now pins the outbound bytes to `"N0CALL\r\n"`, so a real callsign cannot be wired in silently. RBN's plaintext nature is disclosed in the privacy policy and in the Data safety notes.
